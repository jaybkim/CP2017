...
//======================================================Implement below=================================================================//		
	...
	class ButtonListener implements ActionListener{
        ...
        void move(int startX, int startY, int dstX, int dstY) {
            ...
            System.out.printf("move(): %s %s from (%d,%d) to (%d,%d)\n",
                getIcon(dstX, dstY).color, getIcon(dstX, dstY).type, 
        ...
        void showPossibleMove() {
            ...
            for(int k=0;k<8;k++) {
                for(int l=0;l<8;l++)
                    System.out.print(PossibleMove[l][k] ? "M " : ". ");
                System.out.println();
            }
        }
        ...
        boolean StillChecked(int x, int y) { // move to anywhere that can go and check whether still checked
            ...
            System.out.printf("StillChecked(): checking if move %s(%d,%d) escape Check\n", turn, x, y);
            showPossibleMove();

            // make a copy to avoid overwriting
            boolean[][] PossibleMoveCopy = new boolean[8][8];
            for (int i=0; i<8; i++)
                for (int j=0; j<8; j++)
                    PossibleMoveCopy[i][j] = PossibleMove[i][j];

            for(int i=0;i<8;i++) {
                for(int j=0;j<8;j++) {
                    if (PossibleMoveCopy[j][i] == true) {
                        System.out.printf("StillChecked(): checking move %s(%d,%d) to (%d,%d)\n", turn, x, y, i, j);
...
//                            showPossibleMove();
        ...
        void undo(int x, int y, int i, int j, PlayerColor tmpColor, PieceType tmpType) { // getICon(i, j) was overwritten by move(x, y, i, j)
            ...
            System.out.printf("undo(): move %s %s back from (%d,%d) to (%d,%d)\n",
                getIcon(x, y).color, getIcon(x, y).type, 
                i, j, x, y);
            if (tmpType != PieceType.none) {
                System.out.printf("undo(): restore %s %s on (%d,%d)\n",
                    tmpColor, tmpType, i, j);
            }
        }
            ...
            if (isCheck() == false)
                ...
                for(int i=0;i<8;i++) {
                    for(int j=0;j<8;j++) {
                        ...
                        {
                            System.out.printf("isCheckMake(): checking if moving %s(%d,%d) escape Check\n", turn, i, j);
                            if (StillChecked(i, j) == false) {
                            	System.out.printf("isCheckMake(): moving %s (%d,%d) can escape Check\n", turn, i, j);
                                return false;
                            }
                        }
                    }
